...
pub(crate) enum RekeyInitiatorError {
    ...
    AckWrongSequence,
    #[error("operator rekey pending key no longer matches the committed session key")]
    PendingKeyDrift,
...
/// material. While an Ack is pending, the exact candidate key is re-derived
/// transiently from the caller-owned rekey root and immediately placed in a
/// one-shot `xenia_wire::Session`, whose key storage zeroizes on drop.
...
impl OperatorRekeyInitiator {
    ...
    /// Prepare the next interval-driven Proposal while the live `Session` still
    /// uses the old key. Every fallible local step happens here, before the
    /// transport send. On success the Proposal envelope is ready to send but the
    /// live key has not changed yet.
    pub(crate) fn prepare_interval(
        ...
        let proposal_envelope = match session.seal(
            &proposal_bytes,
    ...
    /// Verify and consume the outstanding Ack.
    ///
    /// The Ack is opened in a one-shot session containing *only* the exact new
    /// key derived for the outstanding epoch. The live session's previous-key
    /// grace list is never consulted, closing the old-key forged-Ack ambiguity.
    pub(crate) fn accept_ack(
        ...
        let mut expected_key =
            xenia_handshake::derive_operator_rekey_key(rekey_root, &pending.epoch_hash);
        if key_digest(&expected_key) != self.current_key_digest {
            expected_key.fill(0);
            return Err(RekeyInitiatorError::PendingKeyDrift);
        }
        let mut verifier = Session::with_source_id(self.source_id, self.session_epoch);
        verifier.install_key(expected_key);
        expected_key.fill(0);
...
mod tests {
    ...
    const SOURCE_ID: [u8; 8] = *b"xnaopch1";
    ...
    fn live_session() -> Session {
        let mut session = Session::with_source_id(SOURCE_ID, SESSION_EPOCH);
    ...
    #[test]
    fn prepare_is_old_key_only_and_commit_is_one_way_to_pending_ack() {
        let mut session = live_session();
        let mut rekey = initiator();

        rekey.prepare_interval(&mut session, &REKEY_ROOT).unwrap();
        ...
        assert_eq!(rekey.commit_sent(&mut session, deadline).unwrap(), 1);
    ...
    fn exact_new_key_ack_is_required_to_confirm() {
        ...
        rekey.prepare_interval(&mut session, &REKEY_ROOT).unwrap();
        rekey
            .commit_sent(&mut session, Instant::now() + Duration::from_secs(1))
            .unwrap();
        ...
        let ack = ack_envelope(new_key, key_epoch, epoch_hash, false);

        assert_eq!(rekey.accept_ack(&ack, &REKEY_ROOT, Instant::now()).unwrap(), 1);
        assert!(rekey.application_allowed());
        assert!(!rekey.is_pending_ack());
    }

    #[test]
    fn old_key_forged_ack_is_rejected_even_during_session_grace() {
        let mut session = live_session();
        let mut rekey = initiator();
        rekey.prepare_interval(&mut session, &REKEY_ROOT).unwrap();
        rekey
            ...
            .unwrap();
        let (key_epoch, epoch_hash) = pending_identity(&rekey);
        let forged = ack_envelope(INITIAL_KEY, key_epoch, epoch_hash, false);

        assert_eq!(
            rekey.accept_ack(&forged, &REKEY_ROOT, Instant::now()),
    ...
    fn matching_ack_after_deadline_is_rejected() {
        ...
        rekey.commit_sent(&mut session, deadline).unwrap();
        let (key_epoch, epoch_hash) = pending_identity(&rekey);
        let new_key = xenia_handshake::derive_operator_rekey_key(&REKEY_ROOT, &epoch_hash);
        let ack = ack_envelope(new_key, key_epoch, epoch_hash, false);

        assert_eq!(
            rekey.accept_ack(&ack, &REKEY_ROOT, deadline + Duration::from_millis(1)),
    ...
    fn ack_must_be_first_new_key_envelope() {
        ...
        let (key_epoch, epoch_hash) = pending_identity(&rekey);
        let new_key = xenia_handshake::derive_operator_rekey_key(&REKEY_ROOT, &epoch_hash);
        let ack = ack_envelope(new_key, key_epoch, epoch_hash, true);

        assert_eq!(
            rekey.accept_ack(&ack, &REKEY_ROOT, Instant::now()),
    ...
    fn wrong_ack_identity_is_rejected_without_advancing_lineage() {
        ...
        let (key_epoch, epoch_hash) = pending_identity(&rekey);
        let new_key = xenia_handshake::derive_operator_rekey_key(&REKEY_ROOT, &epoch_hash);
        let ack = ack_envelope(new_key, key_epoch + 1, epoch_hash, false);
    ...
    fn zero_rekey_root_fails_before_consuming_a_nonce() {
        let mut session = live_session();
        let mut rekey = initiator();
        let before = session.nonce_counter();

        assert_eq!(
            rekey.prepare_interval(&mut session, &[0u8; 32]),
            Err(RekeyInitiatorError::ZeroRekeyRoot)
        );
        assert_eq!(session.nonce_counter(), before);
        assert!(rekey.application_allowed());
    }
}
