...
//! Runtime typestate joining SIF semantic receive verification to durable custody.
//!
//! `xenia-ledger` owns the protected protocol and portable evidence semantics while
//! `xenia-peer-core` owns private staging and filesystem publication. This module is
//! the application-layer join between those responsibilities.
//!
//! One authenticated protected Chunk advances both semantic verification and disk
//! staging through a move-only transition. If either side fails, the runtime value is
//! consumed and private staging is dropped, so callers cannot continue from split
//! semantic/disk state.
//!
//! Positive custody is also a joined state: [`DurableVerifiedSifReceive`] can only be
//! constructed after exact whole-file verification and successful crash-durable
...
use xenia_ledger::{
    ...
    SifDeliveryReceiptError, SifProtectedFileChunk, SifProtectedFileComplete,
    SifProtectedFileOffer, SifProtectedFileReceiveError, SifProtectedFileReceiveTerminal,
    SifProtectedFileReceiver, SignatureSuite, VerifiedSifPersistenceOutcome,
    VerifiedSifProtectedFileReceive,
};
use xenia_peer_core::{
...
///
/// Construction derives the final destination basename from the authenticated Offer;
/// callers provide only a receive directory. Because the Offer's validated basename
/// forbids path separators, NUL, `.` and `..`, runtime code cannot silently publish the
/// protected content under an unrelated caller-selected filename.
...
impl SifReceiveRuntime {
    /// Start semantic verification and private disk staging under `receive_directory`.
    ///
    /// The protected Offer is validated before the final path is derived or a staging
    /// inode is created. The final path is exactly
    /// `receive_directory.join(offer.display_name())`.
    pub fn begin(
        ...
        let final_path = receive_directory.join(semantic.offer().display_name());
        let staging =
    ...
    /// to the private disk stager. Any failure consumes `self`; callers cannot retry
    /// from the same runtime value after only one side of the bridge advanced.
    pub fn accept_chunk(
        mut self,
    ...
    ///
    /// This preserves the distinction between content custody and protocol
    /// finalization: exact verified content can still be durably published, while an
    /// incomplete or mismatched stream remains typed negative evidence.
...
impl PersistenceFailedSifReceive {
    ...
    /// Local persistence failure or post-publication durability uncertainty.
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn authenticated_offer_name_selects_final_local_basename() {
        let payload = b"abcdefghij";
        let offer = offer_for(payload);
        let dir = temp_dir();
        let runtime = SifReceiveRuntime::begin(offer, &dir).unwrap();
        assert_eq!(runtime.offer().display_name(), "evidence.bin");
        assert!(!dir.join("evidence.bin").exists());
        drop(runtime);
    ...
    fn same_chunks_drive_semantic_and_disk_frontiers_to_durable_custody() {
        ...
        let runtime = SifReceiveRuntime::begin(offer.clone(), &dir).unwrap();
        let runtime = runtime
    ...
    fn incomplete_semantic_terminal_never_publishes_final_name() {
        ...
        let dir = temp_dir();
        let path = dir.join("evidence.bin");
        let runtime = SifReceiveRuntime::begin(offer.clone(), &dir)
            .unwrap()
            .accept_chunk(&SifProtectedFileChunk::new(&offer, 0, b"abcd".to_vec()).unwrap())
            .unwrap();
        assert!(matches!(
