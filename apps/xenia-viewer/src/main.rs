...
struct Args {
    ...
    /// binary, or via --gui from a desktop shortcut on Windows), falls
    /// back to `xenia-viewer.log` next to the executable -- otherwise
    /// nothing durable would capture a crash or early error.
    #[arg(long)]
