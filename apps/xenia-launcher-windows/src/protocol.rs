...
//! Command/event protocol between the GUI thread (owns the Win32 message
//! loop, the tray icon, and the settings window) and the worker thread
//! (owns a tokio runtime and drives `xenia_launcher_core`).
//!
//! Split across two threads because `tray-icon`/the Win32 message loop and
//! `xenia_launcher_core`'s async process/health/log-tail APIs each want to
//! own their own event loop -- rather than force one into the other, the
//! GUI thread sends [`Command`]s and polls for [`Event`]s via plain
//! `std::sync::mpsc` channels (not tokio channels: the GUI side is
//! deliberately synchronous, driven by the Win32 message loop's own
...
pub enum Event {
    ...
    /// Something worth surfacing to the user via a balloon notification --
    /// deliberately a separate event from `StatusChanged` (see
    /// `src/notify.rs`'s doc comment): not every status change deserves an
    /// interruption, but some do (e.g. the daemon exiting unexpectedly).
    Notify {
        title: String,
        message: String,
    },
