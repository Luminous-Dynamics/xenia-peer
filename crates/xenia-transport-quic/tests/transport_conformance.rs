...
where
    ...
    C: Transport,
{
    let mut host = Session::with_fixture(SessionRole::Host, [0xBD; 8], 0x01);
    let mut viewer = Session::with_fixture(SessionRole::Viewer, [0xBD; 8], 0x01);
    host.install_key([0x88; 32]);
