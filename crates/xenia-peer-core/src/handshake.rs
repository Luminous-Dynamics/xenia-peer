...
pub enum CapabilityAcceptanceError {
    ...
    /// The advertised capability or lane-envelope contract is unsupported.
    #[error(
        "unsupported capability contract: schema={schema_version}, lane_version={lane_envelope_version}, lane_magic={lane_envelope_magic:?}"
    )]
    UnsupportedContract {
        /// Advertised capability schema version.
        schema_version: u16,
...
///
/// The first capabilities frame is checked against the current schema and the
/// context hash authenticated by the handshake. Any later capabilities frame
/// is rejected, including an apparently narrower one: capability changes need
/// a fresh handshake and consent ceremony rather than an in-band downgrade.
#[derive(Debug, Clone)]
...
impl SessionCapabilityGuard {
    ...
    pub fn accept(
        ...
        if !capabilities.supports_current_capability_contract() {
            return Err(CapabilityAcceptanceError::UnsupportedContract {
...
mod tests {
    ...
    fn capability_guard_accepts_exactly_one_authenticated_contract() {
        ...
        let expected = negotiated_session_context_hash(
    ...
    #[test]
    fn capability_guard_rejects_context_mismatch_and_unsupported_contracts() {
        let capabilities = test_capabilities();
        ...
        assert!(matches!(
            ...
            Err(CapabilityAcceptanceError::UnsupportedContract { .. })
        ));
        assert!(!guard.is_accepted());
    }
