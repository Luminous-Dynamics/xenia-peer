...
pub struct TransferSource {
    ...
    // to unlink an owned staged path (important on Windows). The leading
    // underscore documents that ownership/drop behavior is the field's purpose.
    _cleanup_path: CleanupPath,
}
...
impl TransferSource {
    ...
    pub fn from_memory(data: Vec<u8>) -> Self {
        ...
        Self {
            ...
            end_verified: false,
            _cleanup_path: CleanupPath(None),
        }
    }
    ...
    pub async fn next_chunk(
        ...
        let data = match &mut self.kind {
            TransferSourceKind::Memory(data) => {
                let start =
                    usize::try_from(offset).map_err(|_| TransferSourceError::LengthChanged {
                        expected: self.size,
                        observed: offset,
                    })?;
                let end = start
...
mod tests {
    ...
    async fn source_detects_same_length_mutation_after_offer() {
        ...
        let mut source = TransferSource::open_file_limited(&path, 1024)
            .await
            .unwrap();
