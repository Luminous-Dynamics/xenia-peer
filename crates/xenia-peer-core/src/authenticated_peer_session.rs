...
//! authenticated it.
//!
//! [`AuthenticatedPeerSessionV1`] closes that type-level gap. Its fields and
//! constructor are private to this module, and the only public creation path is
//! [`perform_host_handshake_authenticated_peer_session`], which obtains both the
...
pub struct AuthenticatedPeerSessionV1 {
    outcome: HandshakeOutcome,
    peer_identity: VerifiedPeerIdentity,
}

impl AuthenticatedPeerSessionV1 {
    ...
    /// authenticated by this session's host handshake.
...
/// Perform the real host-side hybrid handshake and return a sealed session
/// binding the completed outcome to the exact peer key pair authenticated by
...
pub async fn perform_host_handshake_authenticated_peer_session<T: Transport>(
    transport: &mut T,
    manager: &mut HandshakeManager,
    peer_id: &str,
    negotiated_context_hash: Option<[u8; 32]>,
) -> Result<AuthenticatedPeerSessionV1, Box<dyn std::error::Error>> {
    ...
    Ok(AuthenticatedPeerSessionV1::from_authenticated_parts(
        outcome,
        peer_identity,
    ))
}
...
mod tests {
    ...
    #[test]
    fn sealed_wrapper_preserves_exact_authenticated_parts_without_relabeling() {
        let outcome = HandshakeOutcome {
            session_key: [0x11; 32],
            transcript_hash: [0x22; 32],
            key_schedule: SessionKeySchedule {
                host_to_viewer_control: [0x31; 32],
                viewer_to_host_control: [0x32; 32],
                host_to_viewer_input: [0x33; 32],
                viewer_to_host_input: [0x34; 32],
                host_to_viewer_file: [0x35; 32],
                viewer_to_host_file: [0x36; 32],
                host_to_viewer_media: [0x37; 32],
                viewer_to_host_media: [0x38; 32],
            },
            negotiated_context_hash: Some([0x44; 32]),
            host_identity_fingerprint: [0x55; 32],
        };
        let identity = VerifiedPeerIdentity {
            ed25519_pk: [0x66; 32],
            ml_dsa_pk: vec![0x77; 1952],
        };

        let session = AuthenticatedPeerSessionV1::from_authenticated_parts(
            outcome,
            identity.clone(),
        );

        assert_eq!(session.outcome(), &outcome);
        assert_eq!(session.peer_identity(), &identity);
        assert_eq!(session.transcript_hash(), [0x22; 32]);
        assert_eq!(session.negotiated_context_hash(), Some([0x44; 32]));
        assert_eq!(session.peer_ed25519_public_key(), [0x66; 32]);
        assert_eq!(session.peer_ml_dsa_public_key(), identity.ml_dsa_pk.as_slice());
    }
}
