...
use xenia_handshake_v2_contract::{
    HostFinalizeTranscriptError, HostFinalizeV2, HostHelloV2, ML_DSA_65_PK_LEN,
    ML_DSA_65_SIG_LEN, ML_KEM_768_CT_LEN, ML_KEM_768_PK_LEN, ViewerResponseV2,
    compose_v5_context, encode_host_finalize_v2, encode_host_hello_v2,
    encode_viewer_response_v2, host_signature_transcript_v2, viewer_signature_transcript_v2,
};
...
fn representative_v2_message_and_signature_transcript_vectors_are_frozen() {
    ...
    let host_transcript = host_signature_transcript_v2(
        &host_bytes,
        &response,
        &finalize.final_v5_context_hash,
    )
    .unwrap();
    ...
    assert!(matches!(
        host_signature_transcript_v2(&host_bytes, &response, &mismatched_finalize_v5),
        Err(HostFinalizeTranscriptError::FinalV5ContextMismatch)
    ));

    assert!(matches!(
        host_signature_transcript_v2(&host_bytes, &response, &[0u8; 32]),
        Err(HostFinalizeTranscriptError::Contract(_))
    ));
}
