...
printf 'kind\tpoint\texpected\toutcome\thealth\tadmission_proof_digest\teffect_armed_proof_digest\tcounts\textra\n' \
  > "$EVIDENCE/c0-c10.tsv"
printf 'kind\tdelay_seconds\trepetition\texpected\toutcome\thealth\tadmission_proof_digest\teffect_armed_proof_digest\tcounts\tchild_alive_at_kill\n' \
...
# Real two-process ownership / stale-writer recovery probe.
{
  ...
  kill -0 "$holder" 2>/dev/null || fail "writer exited before competing-open probe"
...
run_deterministic_case() {
  ...
  local point="$2"
  local expect="$3"
  local tmp db crash_log inspect_log cmd inspect_cmd
  tmp="$(mktemp -d)"
  chmod 700 "$tmp"
  db="$tmp/operations-v2.sqlite3"
...
run_commit_race() {
  ...
  local delay="$2"
  local repetition="$3"
  local tmp db ready go child alive inspect_cmd cmd inspect_log child_log
  tmp="$(mktemp -d)"
  chmod 700 "$tmp"
  db="$tmp/operations-v2.sqlite3"
  ready="$tmp/ready"
  ...
  for _ in $(seq 1 1000); do
    ...
    if ! kill -0 "$child" 2>/dev/null; then
      ...
      fail "${kind} commit-race child exited before READY"
    fi
    sleep 0.001
  done
  ...
  counts="${counts//$'\t'/ }"
  printf '%s\t%s\t%s\teither\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$kind" "$delay" "$repetition" "$outcome" "$health" \
    "$admission_proof" "$arm_proof" "$counts" "$alive" >> "$EVIDENCE/commit-races.tsv"
  rm -rf "$tmp"
}
...

{
  ...
  echo "ADMISSION_COMMITTED=$(awk -F '\t' 'NR>1 && $1=="admission" && $4=="committed" {n++} END {print n+0}' "$EVIDENCE/commit-races.tsv")"
  echo "ADMISSION_ABSENT=$(awk -F '\t' 'NR>1 && $1=="admission" && $4=="absent" {n++} END {print n+0}' "$EVIDENCE/commit-races.tsv")"
  echo "EFFECT_ARMED_COMMITTED=$(awk -F '\t' 'NR>1 && $1=="effect-armed" && $4=="committed" {n++} END {print n+0}' "$EVIDENCE/commit-races.tsv")"
  echo "EFFECT_ARMED_ABSENT=$(awk -F '\t' 'NR>1 && $1=="effect-armed" && $4=="absent" {n++} END {print n+0}' "$EVIDENCE/commit-races.tsv")"
} > "$EVIDENCE/summary.txt"

sha256sum \
  "$EVIDENCE/environment.txt" \
  "$EVIDENCE/sqlite-source.txt" \
  "$EVIDENCE/c0-c10.tsv" \
  "$EVIDENCE/commit-races.tsv" \
  "$EVIDENCE/summary.txt" \
