...
- V21 streams inbound file bodies through a shared `IncomingFileStager` on daemon, native viewer, and mobile instead of accumulating whole-file receive buffers. Chunks must be exactly sequential, BLAKE3 is updated incrementally, completion requires exact size/hash, publication is same-directory/no-clobber, partial staging is removed on teardown, and narrowly matched prior-process staging orphans are scavenged on startup. Mobile staged outbound commands are now dequeued only after the authenticated application surface exists and the sender is idle; Android receive and outbound-staging roots are explicitly separated into private app storage.
