...
The persistence adapter is given the exact current frontier claim and must return success only after establishing that this exact state is durably recoverable under the named policy.

**Important integrity precondition:** durable presence is not the same as cryptographic ledger validity.

A restore integration must also run the appropriate Xenia integrity verification before returning success:
...
V0.1 deliberately does not pretend the opaque durability token alone proves those cryptographic checks occurred. The concrete production persistence adapter should combine both durability and restore-integrity checks before minting the token.
...
A callback that falsely reports `Persisted` can falsely mint a token. That is why the production adapter, its exact implementation identity, and its crash qualification belong in the persistence-policy digest/evidence set.
...
- successful restored frontier verification mints a token matching the exact current chain.
