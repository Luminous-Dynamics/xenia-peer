...
| **M2** | Input injection + consent-ceremony UI | 🟡 consent-ceremony gate wired end-to-end 2026-07-02 (**B3**). Input: `XdgPortalInjector` (`org.freedesktop.portal.RemoteDesktop` via `ashpd`) landed 2026-07-02 and validated on KDE-Wayland — 8/8 real pointer/key/touch injections succeeded (see `mycelix-sovereign/docs/input-injection-validation-runbook.md`). Daemon receive-loop landed 2026-07-02: `Transport` split into `SendEnvelope`/`RecvEnvelope` halves for all 3 backends (TCP/WS/QUIC), `LaneSession::seal_input_event`/`open_input` added (control-lane key, no `XLN1` wrap), and `apps/xenia-peer` now runs a dedicated recv task that dispatches lane-tagged control frames (rekey acks, via a channel `perform_rekey_split` awaits) vs. bare envelopes (input, gated through `M1RuntimeSession::allow_input_flow` before reaching the new `--input-backend {noop,log,xdg-portal}` injector, constructed lazily so a view-only `noop` session never triggers the portal consent dialog). Verified live: real daemon+viewer over TCP, 22 frames byte-exact + 6 rekeys through the new split/ack-channel path, clean exit. **Still missing**: `apps/xenia-viewer/src/gui.rs` has zero real egui mouse/keyboard capture wired ("that's M2" — still true, just for the viewer side now). `WaylandInputInjector`/`UinputInjector` remain scaffold stubs. |
