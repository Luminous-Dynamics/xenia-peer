...
| CI (GitHub Actions) | ✅ | fmt + clippy + test + MSRV 1.94 + docs + h264 matrix, all on `ubuntu-latest` (corrected 2026-07-30 -- the prior "(ubuntu/macos/windows)" here was stale/inaccurate: every job in `.github/workflows/ci.yml` ran on `ubuntu-latest` only, no `macos-latest` runner exists anywhere in this repo's workflows). `windows-latest` now exists as one scoped job (`xenia-secure-file`'s own test+clippy, for its real `#[cfg(windows)]` Win32 ACL/reparse-point backend -- see ADR-003) -- not the full workspace |
...
| T3.4 | `crates/crates/symthaea-phone-embodiment/src/scrcpy/*` + `streaming_bridge.rs` | ~2000 | ✅ **done 2026-07-05** — new AGPL-3.0-or-later `crates/xenia-capture-scrcpy` (matching the `xenia-ledger` licensing-isolation precedent, since this is a straight port of AGPL-derived Symthaea code, not a fresh permissive-license implementation). Ports symthaea-phone-embodiment's hardware-proven `scrcpy/{mod,wire,decoder,stream}.rs` (protocol/wire/HEVC-decode layer only -- none of `streaming_bridge.rs`/`bridge.rs`/`adb.rs`'s Symthaea-specific coupling was needed) behind `ScrcpyScreenCapture`, a `xenia_capture::ScreenCapture` impl requiring only ~200 LOC of new glue (`capture.rs`+`lib.rs`). New `--phone-serial <adb-serial>` / `--phone-tcp-port` flags on `xenia-peer` (gated behind a new `scrcpy` feature), taking priority over the scap/TestCapture desktop backends when set. One real compile-time fix needed: `ffmpeg::software::scaling::Context` (inside the ported `HevcDecoder`) isn't `Send` by default, which `ScreenCapture: Send` requires -- added a justified `unsafe impl Send for HevcDecoder` (single-owner/single-consumer usage, no concurrent-access hazard; the workspace's `unsafe_code = "deny"` lint doesn't apply since this new crate doesn't opt into `[lints] workspace = true`). **Verified live against the real Pixel 8 Pro** (serial `41201FDJG000UM`, native resolution 1008×2244): a real daemon (`--phone-serial ... --codec h264`) and real viewer, with real `adb shell input swipe` gestures generating genuine on-screen motion, captured/encoded/transmitted/decoded **60 real frames in ~15s (~4fps sustained)**, the viewer hitting its `--frames 60` target and exiting cleanly. **Real finding along the way**: `--codec passthrough` (uncompressed, ~9MB/frame at this resolution) and `--codec hdc` both produced only 0-1 real frames per 15-25s window despite active swiping and a confirmed-unlocked screen (`dumpsys window` showed `isKeyguardShowing=false`) -- an encode-side bottleneck at this resolution, not a defect in the scrcpy capture/decode layer (which worked correctly in every codec configuration, at least one real frame flowed end-to-end even in the slow ones). Root-caused and fixed the HDC half of that finding -- see **M4.6**, a real ~2,048x algorithmic reduction in the codec's per-tile change-detection cost, not just a symptom of host contention. **H.264 remains the best-verified codec pairing for phone capture at native resolution today**; HDC should be substantially better post-M4.6 but a live phone re-confirmation is still pending a quieter host window (task #32). |
...
| **M4.3b** | Opus audio payload | ✅ done -- this row was stale (see **T3.1** below, which already documents Opus as implemented, feature-gated (`audio-opus` on both `xenia-peer`/`xenia-viewer`), and verified live against real hardware on 2026-07-03: real mic capture + real Opus encode + WS transport + real Opus decode + real speaker playback, 20 frames decoded/18 played at 48kHz stereo, same result as the raw-PCM run just compressed) |
...
| **M4.5** | Phone-as-source capture via scrcpy (T3.4) | ✅ **done 2026-07-05** — new `xenia-capture-scrcpy` crate, `--phone-serial`/`--phone-tcp-port` flags, verified live against a real Pixel 8 Pro (60 frames, ~4fps, H.264, 1008×2244). Closes the "Phone → Desktop" leg of the three-way test matrix. See T3.4 for full detail. |
| **M4.6** | HDC codec: fix a real per-frame algorithmic bottleneck | ✅ **done 2026-07-05/06** — root-caused why `--codec hdc` starved to 0-1 real frames per 15-25s window in the M4.5 live phone test (1008×2244, 576 64×64 tiles): `encode_tile_hdc`/`ContinuousHV::similarity` materialized and compared a dense `TILE_HDC_DIM`=16,384-element vector **per tile, per frame**, purely to compute a cosine-similarity change-detection score. Traced the math: since a tile is only ever compared against *itself* at a later frame (never cross-position), the same per-tile "position" vector appears on both sides of every similarity call, and the comparison provably collapses to an exact 8-dimensional weighted reduction (`Σ_band w_a[band]·w_b[band]·S[band]`, where `S[band]` is a per-tile-position constant computable **once at encoder construction**, not per frame) — not an approximation, an exact ~2,048x reduction in the per-tile hot-loop cost. Replaced the dense-vector `ContinuousHV` machinery with `TileFeatures` (8 scalars) + `weighted_cosine_similarity` + `generate_band_sumsq` (one-time construction cost). New `weighted_cosine_similarity_matches_dense_hv_reduction` test directly proves bit-for-bit equivalence against the original dense-vector formula. Wire format, `HdcEncoder`/`HdcDecoder` public API, and all pre-existing tests (16/16) untouched and passing. **Measured real speedup**: new `examples/hdc_throughput.rs` (isolates the codec's own CPU cost from scrcpy/ADB/network) at 1008×2244/60 synthetic frames, `--release`, on the *same* heavily-contended shared machine (load avg ~54 on 12 cores) both before and after: 680.54ms/frame (1.5fps) → 139.17ms/frame (7.2fps), a real ~4.9x mean speedup. The remaining per-frame floor is dominated by per-pixel feature extraction (2.36M pixel reads/frame) and host contention, not the (now near-free) similarity math. **Live phone re-confirmation attempted 2026-07-06 after load avg dropped 54→12, still inconclusive** — 3 consecutive live `--codec hdc` runs against the real Pixel 8 Pro (real dynamic home-screen content this time, confirmed via `dumpsys`/screenshot, not the static blank-tab mistake of the first attempt) each produced **zero** real frames sent in a 20-40s window, while an interleaved `--codec h264` control run on the identical capture path succeeded immediately (60 frames). A temporary timing probe placed directly around `encoder.encode()` never logged even once — the very first (keyframe) `encode()` call itself never returned — while `top -H` on the daemon PID showed sustained 75-90% single-core CPU the whole time (genuinely computing, not blocked on I/O). Root cause is **not this fix**: `vmstat`/`/proc/pressure/memory` showed the host was under real, *currently active* swap thrashing (up to 28MB/s swap-out, nonzero "full" PSI stalls) throughout every attempt, despite load average looking calm -- load average and memory pressure are different signals and this host had low load but a swapped-out working set. HDC's still-real per-tile allocation pattern (576 small `Vec<u8>` allocs/frame from `extract_tile_rgb`) is exactly the shape of workload that gets catastrophically slow under active page-fault/swap pressure, while H.264's lighter allocation profile isn't. **Task #32 updated accordingly: check `vmstat 1`/`/proc/pressure/memory` for active swap-out, not just `uptime`'s load average, before the next live-HDC attempt.** The M4.6 fix itself remains proven (equivalence test + isolated release-mode benchmark, both machine-load-independent); only the live end-to-end confirmation is still blocked. |
| **M4.4** | Migrate xenia-viewer-web frontend to Leptos | ✅ **done 2026-07-05** (in the `xenia-wire` repo, new `xenia-viewer-web/leptos-app` Trunk-built CSR crate) — wraps the existing `xenia-viewer-web` WASM handshake/lane-session/decode APIs (`WasmHandshake::begin_inner`/`finish_inner`, `WasmLaneSession`, `WasmRekeyState`, `open_lane_frame_inner`) in a small reactive UI; connection state and frame count are `RwSignal`s, but the `web_sys::WebSocket`-holding `Connection` itself is a plain `Rc<RefCell<Option<Connection>>>` outside the signal system (it isn't `Send`, which `RwSignal`'s default `SyncStorage` requires even in single-threaded WASM). Needed `#[wasm_bindgen(start)] fn main()` + `#![no_main]` at the crate root — a plain `fn main()` in a wasm32 bin-target crate is never invoked by wasm-bindgen's JS glue, and the attribute alone conflicts with Rust's implicit binary entry point without `#![no_main]`. Verified live: real `xenia-peer --transport ws` daemon, completes the PQC-hybrid handshake, decodes and canvas-renders a real HDC frame (1920×1080). **WebCodecs H.264 decode** — new `h264.rs` ports daemon.js's `handleH264Frame`/`drawVideoFrame`; web-sys's WebCodecs bindings need `--cfg=web_sys_unstable_apis` (added via `.cargo/config.toml`), which also swapped `put_image_data`'s stable f64-arg overload for an i32-arg unstable one (fixed the one call site). `VideoDecoderConfig` has no typed `avc` field in web-sys, set via raw `js_sys::Reflect`. Verified live: real `xenia-peer --transport ws --codec h264` daemon + this Leptos app in real headless Chromium via CDP, 199 real frames decoded through the browser's native `VideoDecoder`, zero errors. Building this crate needs `nix shell nixpkgs#trunk nixpkgs#wasm-bindgen-cli nixpkgs#binaryen` rather than xenia-peer's own flake devShell -- its pinned rustc 1.94.0 is older than this crate's `rust-version = "1.95"`; no native GUI libs needed since it's a WASM build. **Clipboard UI (host-to-viewer, write-only)** — new `OpenedLaneFrame::Clipboard` decode (was falling into the `Other` catch-all), backed by `RawClipboardShadow` mirroring `xenia_peer_core::frame::RawClipboard` byte-for-byte (2 new cross-compat tests). Leptos calls `navigator.clipboard.writeText()` on receipt. Deliberately write-only, not a gap: the native viewer's viewer-to-host reverse direction has no clean browser equivalent (`readText()` needs a permission prompt + document focus, no change event to poll). Verified live via real headless-Chromium CDP that the full pipeline (host clipboard change → seal → send → decode → `writeText` call) reaches the real browser Clipboard API, correctly surfacing `NotAllowedError: Document is not focused` rather than crashing -- an inherent Clipboard-API security requirement headless automation can't satisfy, not a code defect; a real focused tab calls it cleanly. |
...
### ✅ Phone → Desktop (works today, Path B)

**Path B — desktop as daemon of phone's screen** shipped 2026-07-05
(**T3.4** above): `xenia-peer --phone-serial <adb-serial> --codec h264`
runs a real desktop daemon that captures a USB-connected phone's screen
via scrcpy and reframes it as Xenia frames for any viewer (native, or
the phone's own browser looped back through Tailscale). **Verified live**
against a real Pixel 8 Pro: 60 real frames captured/encoded/decoded in
~15s (~4fps) with real on-screen motion (`adb shell input swipe`), at
the device's native 1008×2244 resolution. H.264 is the codec that
actually sustains this at native phone resolution -- passthrough and
HDC both starved to 0-1 frames per test window (encode-side bottleneck,
not a capture-layer defect; see T3.4 for detail).

Path A (phone as daemon, cross-compiled to aarch64-linux-android with
a `MediaProjection`-based capture backend) remains undone and is real,
multi-week product work of its own -- but is no longer needed to close
the three-way test matrix, since Path B fills the same gap with a
fraction of the effort. Worth revisiting only if untethered (no-USB)
phone-as-source becomes a real product requirement.
...
3. ~~**Phone → Desktop leg: Path A or Path B?**~~ Resolved 2026-07-05:
   Path B shipped (`xenia-capture-scrcpy`, T3.4) and closes the three-way
   test matrix. Path A (untethered phone-as-daemon) remains open only
   if no-USB deployment becomes a real requirement.
